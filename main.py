...
imgtit    = "blaumeise"      # without file ending (i.e. no ".jpg", ".png"...)
...
contr_inc = "yes"            # would you like to increase output img contrast (more vibrant)?
...
#######################################################
# get the centroid info for each cluster:
...
def additionalColours(idx_list, all_colours):
    ...
    return col_coll, distrems     # distrem tells us if additional colours were removed due to too small distance
...
def centroidMatcher(pixels):
    ...
    for px in range(0, width, scale_fac):
        for py in range(0, height, scale_fac):
            ...
            rep_r, rep_g, rep_b = final_cols[idx_min]
...
###############################
# sort the final colours by dist to white, so more similar shades appear close (do: potentially HSV cols here)
n = 0
distlist = list()
...
def paletteMaker(frame, summe=tile, n=0):
    for rgb_idx in range(len(final_doms)):
        while n < summe:
            idx = int(summe / tile - 1)
